defmodule Assignment.CoindataCoordinator do
  ...
  def add_coin(coin_name) when is_binary(coin_name) do
    GenServer.cast({:global, {Node.self(), __MODULE__}}, {:add_coin, coin_name})
  end

  def add_coin(coin_name, history_keeper_worker_state)
      when is_map(history_keeper_worker_state) and is_binary(coin_name) do
    GenServer.cast(
      {:global, {Node.self(), __MODULE__}},
      {:add_coin, coin_name, history_keeper_worker_state}
    )
  end

  def get_history_keeper_worker_statistics() do
    GenServer.call({:global, {Node.self(), __MODULE__}}, :get_history_keeper_worker_statistics)
  end
  ...
  def get_hist_keeper_coin_pairs() do
    GenServer.call({:global, {Node.self(), __MODULE__}}, :get_hist_keeper_coin_pairs)
  end
  ...
  def transfer_coin(coin_name, node_to) do
    GenServer.cast({:global, {Node.self(), __MODULE__}}, {:transfer_coin, coin_name, node_to})
  end

  def balance() do
    GenServer.cast({:global, {Node.self(), __MODULE__}}, :balance)
  end
